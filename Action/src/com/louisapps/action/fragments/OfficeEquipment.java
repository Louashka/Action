...
public class OfficeEquipment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.office_equipment, container,
                false);
        
        progressDialog = ProgressDialog.show( getActivity(), "", "Loading", true );
        listView = (ListView) rootView.findViewById( R.id.officeEquipment );
        
        getOfficeEquipmentTasks( new BackendlessCallback<BackendlessCollection<ActionCreation>>()
        	    {
        	      @Override
        	      public void handleResponse( BackendlessCollection<ActionCreation> taskBackendlessCollection )
        	      {
        	    	  for( ActionCreation task : taskBackendlessCollection.getCurrentPage())
        	    		  userActionsList.add(new AllCategoriesItems (task.getNumberImage(), task.getNameAction()
        	    				  + "\n" + task.getNameCompany() + "\nс " + task.getAction_start() 
        	    				  + "\nпо " + task.getAction_end() + "\n" + task.getAdress() + "\n" + task.getDescrip()));       	    	
	...
	public static void getOfficeEquipmentTasks( final AsyncCallback<BackendlessCollection<ActionCreation>> responder )
	  {
		String whereClause = "category = 'Компьютеры и оргтехника'";
